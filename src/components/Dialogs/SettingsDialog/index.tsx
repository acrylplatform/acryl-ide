...
export default class SettingsDialog extends React.Component<ISettingsDialogProps> {
    ...
    handleAdd = () => this.props.settingsStore!.addNode({url: 'https://nodestestnet.acrylplatform.com/', chainId: 'K'});
        ...
        >
            <Scrollbar className={styles.content}>
                ...
                {this.props.settingsStore!.systemNodes.map((node, i) => (
                    <NodeItem key={i} node={node} index={i} title={node.chainId === 'A' ? 'Mainnet' : 'Testnet'}/>
                ))}
